...
func CompareItems[T any](source []T, target []T, depth uint) error {
	if depth == 0 {
		return nil
	}

	if len(source) != len(target) {
		return fmt.Errorf("source and target has different length. Source: %v, Target: %v", len(source), len(target))
	}

	for i := range source {
		if err := CompareData(source[i], target[i], depth); err != nil {
